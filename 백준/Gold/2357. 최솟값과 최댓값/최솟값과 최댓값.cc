...
using namespace std;
int n, m, k, a, b;
int arr[1000001], max_tree[4000001], min_tree[4000001];
int in_max(int s, int e, int node) {
    int m = (s+e)/2;
	if(s==e)
        return max_tree[node] = arr[s];
	
	return max_tree[node] = max(in_max(s, m, node*2), in_max(m+1, e, node*2+1));
} 

long long find_max(int s, int e, int node, int l, int r) { 
	int m = (s+e)/2;
    if(s > r || e < l)
        return 0;
	if(l<=s && e <= r)
	...
	return max(find_max(s, m, node*2, l, r), find_max(m+1, e, node*2+1, l, r));
}
