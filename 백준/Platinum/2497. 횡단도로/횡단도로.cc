...
int main() {
    ...
    for(int i=0;i<s.size();i++){
        ...
        if(ans < (arr[s[i]].x-arr[s[j]].x)*(arr[s[i]].x-arr[s[j]].x)+(arr[s[i]].y-arr[s[j]].y)*(arr[s[i]].y-arr[s[j]].y)){
            ans = (arr[s[i]].x-arr[s[j]].x)*(arr[s[i]].x-arr[s[j]].x)+(arr[s[i]].y-arr[s[j]].y)*(arr[s[i]].y-arr[s[j]].y);
            a1 = arr[s[i]].x;
            a2 = arr[s[i]].y;
            a3 = arr[s[j]].x;
            a4 = arr[s[j]].y;
