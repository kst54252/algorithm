...
bool com(POINT &p, POINT &q){
    if(p.y_d * q.x_d != p.x_d * q.y_d)
        return p.y_d * q.x_d < p.x_d * q.y_d;
    if(p.y != q.y)
        return p.y < q.y;
    return p.x < q.x;
}
...
int main() {
    ...
    if(s.size() == 2){
        cout << fixed << setprecision(10) << sqrt((arr[s[0]].x-arr[s[1]].x)*(arr[s[0]].x-arr[s[1]].x)+(arr[s[0]].y-arr[s[1]].y)*(arr[s[0]].y-arr[s[1]].y));
        return 0;
    }
    double ans = 0.0;
    for(int i=0;i<s.size();i++){
        ...
        ans = max(ans, sqrt((arr[s[i]].x-arr[s[j]].x)*(arr[s[i]].x-arr[s[j]].x)+(arr[s[i]].y-arr[s[j]].y)*(arr[s[i]].y-arr[s[j]].y)));
    }
